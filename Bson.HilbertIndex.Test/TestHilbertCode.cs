...
namespace Bson.HilbertIndex.Test
{
    ...
    public class Tests
    {
        ...
        public void Index_can_Find_Nearest_Neighbor()
        {
            ...
            var neareast = index.NearestNeighbors(new Coordinate(18.0001, 57.0001)).First();
            Assert.That(neareast.Id, Is.EqualTo(1));

            neareast = index.NearestNeighbors(new Coordinate(18.2001, 57.0001)).First();
            Assert.That(neareast.Id, Is.EqualTo(2));

            neareast = index.NearestNeighbors(new Coordinate(18.5001, 57.0001)).First();
            Assert.That(neareast.Id, Is.EqualTo(3));

            // Test exact at edges
            neareast = index.NearestNeighbors(new Coordinate(18, 57)).First();
            Assert.That(neareast.Id, Is.EqualTo(1));

            neareast = index.NearestNeighbors(new Coordinate(18.5, 57)).First();
            Assert.That(neareast.Id, Is.EqualTo(3));

            var far_a_way = new Coordinate(-74, 41);

            // Coord in New York should be closest to the most western
            neareast = index.NearestNeighbors(new Coordinate(-74, 41)).First();
            Assert.That(neareast.Id, Is.EqualTo(1));
        }
